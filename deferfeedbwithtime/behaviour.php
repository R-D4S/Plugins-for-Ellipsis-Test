 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbehaviour_deferfeedbwithtime extends question_behaviour_with_save {
    ...
    public function process_finish(question_attempt_pending_step $pendingstep) {
        ...
        $namecookie = "att".($this->qa->get_database_id())."ques".($this->qa->get_question_id());

        $timeforquest = new stdClass();
        $timeforquest->attempt_id = $this->qa->get_database_id();
        if(isset($_COOKIE[$namecookie])){
            $timeforquest->timeon = $_COOKIE[$namecookie];
        }
        else $timeforquest->timeon = 0;
        $timeforquest->id = $DB->insert_record("time_for_question", $timeforquest);
